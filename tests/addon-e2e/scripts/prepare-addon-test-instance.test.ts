...
test("preparation ignores inherited live workspace and skips source symlinks", () => {
  ...
  try {
    ...
    prepared = prepareAddonTestInstance({ repoRoot: repo, env: { QIUSHUIAI_WORKSPACE: live, QIUSHUIAI_ADDON: "demo" } });
    ...
    expect(formatPreparedEnvironment(prepared).some((s) => s.startsWith("QIUSHUIAI_PI_AGENT_DIR="))).toBe(true);
    expect(formatPreparedEnvironment(prepared).some((s) => s.startsWith("QIUSHUIAI_PROFILE="))).toBe(false);
    ...
    const env = preparedInstanceEnvironment({ ...input, PORTAINER_RELAY: 'live', GITHUB_QIUSHUIAI_BOT_PAT: 'live', QIUSHUIAI_INTERNAL_SECRET: 'live', QIUSHUIAI_E2E_INTERNAL_SECRET: 'test-only' });
    expect(env.PORTAINER_RELAY).toBeUndefined(); expect(env.GITHUB_QIUSHUIAI_BOT_PAT).toBeUndefined();
    expect(env.QIUSHUIAI_INTERNAL_SECRET).toBe('test-only');
