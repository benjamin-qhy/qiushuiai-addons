# qiushuiai-addon-m365

Experimental Microsoft 365 tools for QiushuiAI.

## Install

Requires QiushuiAI `>=2.13.3`.

Open **Settings → Add-Ons** and install **m365** from the catalog. The add-on loads after installation; the former `QIUSHUIAI_ENABLE_M365_EXPERIMENTAL` core gate is not used.
...
- A fresh sign-in shows an explicit consent interstitial unless `QIUSHUIAI_M365_YOLO=1`.
...
A managed host can register `globalThis.__qiushuiai_m365CredentialProviderV1` before the add-on first requests authentication:

```ts
globalThis.__qiushuiai_m365CredentialProviderV1 = async ({ resource, minRemainingSeconds }) => {
...
| `QIUSHUIAI_M365_YOLO=1` | Skip the explicit consent interstitial before authentication navigation |
...
From the `qiushuiai-addons` repository root:
