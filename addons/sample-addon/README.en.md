...
Starter template for qiushuiai add-on developers. Demonstrates the core patterns every settings-aware add-on needs.
...
The browser pane is just a local authenticated client of qiushuiai itself:
...
and status classes. Hosts with the Settings authoring contract from QiushuiAI #1318
...
The runtime entry registers config handlers directly with qiushuiai:
...
QiushuiAI lazily loads installed add-on runtime entries on first config request, so the settings pane works without a slash-command bridge.
...
the repository root, opt in with `QIUSHUIAI_E2E_DISPOSABLE=1`,
