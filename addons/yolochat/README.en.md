...
Open **Settings → Add-Ons** and install **yolochat** from the catalog. The shell examples below assume a source checkout of `qiushuiai-addons`; an installed skill must resolve the sender script from its add-on package root.
...
The envelope is delivered as the `content` field of a standard qiushuiai agent message POST.
