# qiushuiai-addon-drawio-editor

Self-hosted draw.io diagram editor extension for qiushuiai. The add-on version matches the bundled upstream draw.io version; add-on `31.4.2` contains draw.io `v31.4.2`.
...
Uses qiushuiai's `globalThis.__qiushuiai_registerRoute` to serve the draw.io webapp.
A wrapper page at `/drawio/edit?path=...` (also accepted as `/drawio/edit.html`) embeds the editor in an iframe and handles
the postMessage protocol for load/save via the qiushuiai raw file API.
