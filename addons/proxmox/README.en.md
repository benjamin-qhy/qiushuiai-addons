...
Proxmox VE management through one session-scoped `proxmox` tool. Requires QiushuiAI `>=1.8.0`.
...
Non-secret defaults use `/agent/addons/api/proxmox/config` and extension KV. Store the API token secret in the keychain; the default entry is `proxmox/qiushuiai-management-token`. Configure the separate token username, such as `root@pam!qiushuiai`. Hostnames and bare IP addresses are normalised to `https://<host>:8006/api2/json`.
