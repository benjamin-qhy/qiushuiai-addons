...
const config = { graphite_host: "127.0.0.1", graphite_port: 2003, graphite_prefix: "qiushuiai", instance_name: "smith.test" };
...
test("collector exports bounded Graphite paths without chat or secret data", () => {
  ...
  expect(batch.points.map(point => point.path)).toEqual(expect.arrayContaining([
    "qiushuiai.smith_test.compaction.local.fast-summary.selective.single_pass.manual.success.none.attempt.count",
    "qiushuiai.smith_test.compaction.local.fast-summary.selective.single_pass.manual.success.none.duration.ttft_ms",
  ]));
