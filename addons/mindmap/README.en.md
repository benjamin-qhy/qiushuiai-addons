...
A workspace editor for `.mindmap.yaml` and `.mindmap.yml` files. Requires QiushuiAI `>=1.8.0`.

## Install

Open **Settings → Add-Ons** and install **mindmap** from the catalog, then reload QiushuiAI.
...
Files are YAML mindmaps consumed by the bundled browser editor. Keep the `.mindmap.yaml` or `.mindmap.yml` suffix so QiushuiAI selects the pane.
