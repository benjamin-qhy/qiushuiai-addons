...
- **Verified model catalogs** — QiushuiAI's runtime registry supplies capability metadata; exact child `pi --list-models` entries establish executability.
...
Open **Settings → Add-Ons**, install **Delegate**, then reload QiushuiAI when convenient. Installing or updating the package does not activate new code until QiushuiAI reloads.

Delegate self-activates after reload. It may also be listed explicitly in `.qiushuiai/config.json`:
...
A discovered MCP adapter is appended only when explicitly available. A custom comma-separated list of child Pi built-ins is also accepted. A named tool must exist in the child; installed QiushuiAI add-on tools are not inherited automatically.
...
For a PDF or Office document, extract it with the appropriate QiushuiAI tool and delegate the resulting text. Convert unsupported images to PNG, JPEG, GIF, WebP, or BMP first.

## Settings API

The browser pane reads configuration from `/agent/addons/api/delegate/config` and model diagnostics from `/agent/addons/api/delegate/models`. Both are authenticated local QiushuiAI endpoints; Delegate stores no secrets.
...
The pane opts into the shared `settings-addon-*` controls from QiushuiAI #1318.
...
set `QIUSHUIAI_E2E_DISPOSABLE=1`,
`QIUSHUIAI_SETTINGS_CORE_SOURCE=/absolute/path/to/qiushuiai` and an installed
...
- Runtime-only QiushuiAI models cannot be used until the child CLI also lists them.
