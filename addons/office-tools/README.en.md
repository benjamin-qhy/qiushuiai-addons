...
Read OOXML documents and create Office/PDF files from Markdown. Requires QiushuiAI `>=1.8.0`.
...
- PDF uses the bundled `assets/md2pdf.css` and a locally installed Chromium-compatible browser.
...
The package declares `typebox` as a peer and ships every Office-specific runtime module. PDF generation discovers Chromium, Chrome or Edge locally; `QIUSHUIAI_BROWSER_PATH` can set an explicit executable.
