...
First-party QiushuiAI packaging of Rob Zolkos's MIT-licensed [`pi-skill-model-effort`](https://github.com/robzolkos/pi-skill-model-effort) extension.
...
- `effort` — temporarily sets QiushuiAI/Pi thinking level while the skill runs
...
- Explicit `/skill:name` invocations apply the override as soon as QiushuiAI receives the raw slash command.
- Automatically selected skills apply after QiushuiAI reads that skill's `SKILL.md`; the next LLM turn in the same run uses the override.
