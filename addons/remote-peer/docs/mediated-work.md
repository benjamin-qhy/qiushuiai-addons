...
`work_review` supplies an approved/rejected result. Approved capability labels must be a subset of those requested. The response returns over Iroh and is stored at the sender. If an origin chat was supplied, QiushuiAI queues a local notification there.
