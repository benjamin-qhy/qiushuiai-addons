# @qiushuiai/qiushuiai-addon-goal

Codex-style persisted thread goals for QiushuiAI.
...
- Goal state is scoped to the current QiushuiAI chat/session (`chat_jid`).
- Deadline checkpoint provider registration is optional and globally idempotent, so duplicate startup imports cannot install competing providers. Older QiushuiAI cores continue using the legacy Goal loop; non-Goal sessions are unaffected.
- Token accounting uses assistant message usage from QiushuiAI events, then applies Codex-style budget-limit behavior.
...
- Goal completion/stop tools update persisted goal state but do not set QiushuiAI's early `terminate` hint. This prevents tool-only turns from completing without user-visible assistant output.
