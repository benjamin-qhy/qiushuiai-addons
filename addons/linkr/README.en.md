...
Control a computer connected to a Radxa Linkr through QiushuiAI: capture its screen, send keyboard and mouse input, and run short firmware-entry sequences. The `linkr` tool handles input and timing; seven bundled skills guide screen-dependent BIOS, boot and operating-system installation work.

Requires QiushuiAI **3.1.2 or later**, a reachable Linkr and a public API token. The add-on uses the documented snapshot and HID interfaces. It does not implement power control, virtual media or appliance reboot.

## Install

Install **Linkr** from Settings → Add-ons using the [public catalogue](https://benjamin-qhy.github.io/qiushuiai-addons/), or use the [v0.1.0 tarball](https://benjamin-qhy.github.io/qiushuiai-addons/packages/qiushuiai-addon-linkr-0.1.0.tgz). Package name: `@qiushuiai/qiushuiai-addon-linkr`.

Reload QiushuiAI after installation to load the tool, Settings pane and skills. Installing the package sends no requests to a device.

## Configure a device

1. Create an access token in the Linkr web interface under System Settings → Access Token.
2. Store it in QiushuiAI's keychain. Do not paste it into a chat, profile field or command argument.
...
- At most four jobs run at once, with one job per configured origin. Control leases prevent overlapping HID operations for an origin within one QiushuiAI process. Other processes, hostname aliases and physical input can still interfere.
...
- Jobs continue after their start call returns, but do not resume after QiushuiAI restarts. In-memory job lookup and control state are lost on restart; inspect the target before further input.
...
At the v0.1.0 merge ([validation run](https://github.com/benjamin-qhy/qiushuiai-addons/actions/runs/34716571245)):
...
MIT-licensed; see [LICENSE](LICENSE). Attribution for Radxa Linkr material is retained. This is an independent QiushuiAI integration.
