# qiushuiai-addon-voice-pipeline

ESPHome-only voice assistant extension for `qiushuiai`.
...
- Sends transcriptions to the active QiushuiAI chat runtime
...
- Persists conversation turns into `tts:default` in the local qiushuiai message DB

## Install

Open **Settings → Add-Ons** and install **voice-pipeline** from the catalog.

Then configure it via environment variables and reload qiushuiai.

### Secrets (keychain-backed)

This add-on reads secrets from environment variables. In the qiushuiai container,
...
| `QIUSHUIAI_DB` | `/workspace/.qiushuiai/store/messages.db` | |
