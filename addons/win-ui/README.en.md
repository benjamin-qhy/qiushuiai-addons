# @qiushuiai/qiushuiai-addon-win-ui

Windows desktop automation add-on for QiushuiAI.
...
The add-on checks `process.platform` and only registers tools on `win32`. It can be installed in cross-platform QiushuiAI environments without breaking Linux or macOS sessions.
