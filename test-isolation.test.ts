...
test('addon test bootstrap replaces live paths and strips injected service credentials', () => {
  const env = { ...process.env, QIUSHUIAI_TEST_FS_ISOLATION_ACTIVE: '0', QIUSHUIAI_WORKSPACE: '/workspace', HOME: '/home/agent', PORTAINER_RELAY: 'not-real', GITHUB_QIUSHUIAI_BOT_PAT: 'not-real' };
  const root = ensureTestFilesystemIsolation(env);
  try {
    expect(env.QIUSHUIAI_WORKSPACE).toStartWith(root.root + '/');
    expect(env.HOME).toStartWith(root.root + '/');
    expect(env.PORTAINER_RELAY).toBeUndefined(); expect(env.GITHUB_QIUSHUIAI_BOT_PAT).toBeUndefined();
...
test('browser target cannot default to a live instance', () => {
  ...
  expect(requireDisposableTestTarget('http://127.0.0.1:3000', { QIUSHUIAI_E2E_DISPOSABLE: '1' })).toBe('http://127.0.0.1:3000');
});
...
test('CI launches the workspace returned by preparation and uses test-only auth', () => {
  ...
  expect(workflow).toContain('QIUSHUIAI_E2E_DISPOSABLE:');
  expect(workflow).toContain('QIUSHUIAI_E2E_INTERNAL_SECRET:');
  expect(workflow).not.toContain('export QIUSHUIAI_WORKSPACE=$(mktemp -d)');
});

test('CI browser cache survives isolated HOME and workflow PRs cannot deploy', () => {
  const workflow = readFileSync(new URL('./.github/workflows/build.yml', import.meta.url), 'utf8');
  expect(workflow).toContain('echo "PLAYWRIGHT_BROWSERS_PATH=$RUNNER_TEMP/qiushuiai-addon-playwright" >> "$GITHUB_ENV"');
