...
export function requireDisposableTestTarget(value: string | undefined, env: Record<string, string | undefined> = process.env): string {
  if (!value?.trim() || env.QIUSHUIAI_E2E_DISPOSABLE !== "1") {
    throw new Error("Set an explicit E2E URL and QIUSHUIAI_E2E_DISPOSABLE=1 for a disposable test instance; live targets are not defaults.");
  }
