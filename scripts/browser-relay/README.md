...
- QiushuiAI container running with host networking
...
Run inside the QiushuiAI container:

```bash
cd /workspace/qiushuiai-addons/scripts/browser-relay
./install-browser-relay.sh
```

This configures:

- `BROWSER=/workspace/qiushuiai-addons/scripts/browser-relay/browser-relay`
- `BROWSER_RELAY_HOST=localhost`
- `BROWSER_RELAY_PORT=9222`

If the `qiushuiai env` CLI is available, the script uses it so the variables are activated immediately and persisted. Otherwise it appends the exports to `/workspace/.env.sh`.
...
cp /workspace/qiushuiai-addons/scripts/browser-relay/wsl-browser-relay.py ~/.local/bin/
cp /workspace/qiushuiai-addons/scripts/browser-relay/wsl-browser-relay.service ~/.config/systemd/user/
...
```bash
python3 /workspace/qiushuiai-addons/scripts/browser-relay/wsl-browser-relay.py
```
